...
import { CrmDefaultError } from "../models/errors/crm-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | CrmDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | CrmDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new CrmDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new CrmDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
